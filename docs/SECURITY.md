...
- The JWT includes the role via `ClaimTypes.Role` (generated by `JwtTokenService`).
- The frontend mirrors these policies in `AuthContext` (`isAdmin`, `canEdit`, `isApprover`) and hides or disables UI actions accordingly.
