...
export default function DashboardPage() {
  ...
  const { data: failed }      = useQuery({ queryKey: ['executions', 'Failed'], queryFn: () => api.executions.list({ status: 'Failed', pageSize: 1 }) });
  ...
  const stats = [
    ...
    { label: 'Failed',          value: failed?.total ?? 0,     icon: AlertCircle, color: 'text-red-600 bg-red-50',       href: '/executions' },
  ];
