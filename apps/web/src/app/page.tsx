...
const features = [
  {
    ...
    desc: 'Drag-and-drop canvas with 20+ node types, version history, and undo/redo.',
  },
  {
    icon: Zap,
    title: 'Multi-Provider AI',
    desc: 'Route tasks across OpenAI, Anthropic, Azure, and Ollama — switch defaults without a restart.',
  },
  {
    icon: ClipboardList,
    title: 'Forms & Approvals',
    desc: 'Build dynamic forms with AI assistance and pause workflows for human input.',
  },
  {
    icon: Database,
    title: 'External Data Intake',
    desc: 'Pause and wait for external systems to POST data, with field validation and type coercion.',
  },
  {
    icon: Shield,
    title: 'Enterprise Security',
    desc: 'JWT auth, RBAC roles, tenant isolation, and encrypted secrets out of the box.',
  },
  {
    icon: BarChart3,
    title: 'Real-Time Monitoring',
    desc: 'Live execution timeline, node logs, AI chat history, and token usage tracking.',
  },
];
...
const mockNodes = [
  ...
  { id: 'end',      x: 530, y:  80, label: 'End',           color: '#0f172a', icon: StopCircle,  w: 100 },
];

// connections as [sourceId, targetId]
const mockEdges: [string, string][] = [
  ['start', 'ai'], ['start', 'cond'],
  ['ai', 'approval'], ['cond', 'code'],
  ['approval', 'end'], ['code', 'end'],
];
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <Link href="/onboarding" className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-3.5 py-1.5 rounded-lg transition-colors">
              Get started
            </Link>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-indigo-50/70 via-white to-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-10 text-center">
          <div className="inline-flex items-center gap-2 bg-white border border-indigo-100 text-indigo-700 text-xs font-medium px-3 py-1 rounded-full mb-6 shadow-sm">
            <Sparkles size={12} /> Visual designer · AI builder · External data intake
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-slate-900 leading-[1.1] tracking-tight">
            Orchestrate AI workflows{' '}
            <span className="text-indigo-600">at enterprise scale</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 mt-8 mb-10">
            <Link href="/onboarding" className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-6 py-3 rounded-xl text-sm flex items-center justify-center gap-2 transition-colors shadow-sm">
              Start for free <ArrowRight size={16} />
            </Link>
            <Link href="/docs" className="border border-slate-200 hover:bg-slate-50 text-slate-700 font-medium px-6 py-3 rounded-xl text-sm transition-colors">
              Read the docs
            </Link>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5 flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2">
            {providers.map(({ name, color }) => (
              <span key={name} className={`text-xs font-semibold px-3 py-1.5 rounded-lg border ${color}`}>
                {name}
              </span>
            ))}
          </div>
      ...
      <section className="bg-indigo-600">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16 sm:py-20 text-center">
          <h2 className="text-2xl sm:text-3xl font-semibold text-white mb-3">Ready to get started?</h2>
          <p className="text-indigo-200 text-sm mb-7 max-w-md mx-auto">Spin up a workspace in minutes. No infrastructure required.</p>
          <Link href="/onboarding" className="bg-white text-indigo-600 hover:bg-indigo-50 font-medium px-6 py-3 rounded-xl text-sm transition-colors inline-flex items-center gap-2">
