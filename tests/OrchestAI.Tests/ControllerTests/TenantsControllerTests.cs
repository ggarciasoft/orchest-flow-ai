...
public sealed class TenantsControllerTests
{
    ...
    public async Task AcceptInvite_ValidToken_Returns200AndCreatesUser()
    {
        ...
        userRepo.Setup(r => r.CreateAsync(It.IsAny<OrchestAI.Domain.Entities.User>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((OrchestAI.Domain.Entities.User u, CancellationToken _) => u);
        ...
        userRepo.Verify(r => r.CreateAsync(
            It.Is<OrchestAI.Domain.Entities.User>(u => u.Email == "carol@example.com" && u.Role == UserRole.Editor),
