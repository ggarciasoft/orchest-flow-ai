...
public sealed class NodesController : ControllerBase
{
    ...
    public ActionResult<object> Catalog()
    {
        var descriptors = _registry.GetAllDescriptors().Select(d => new NodeDescriptorResponse(
            ...
            d.Configuration.Select(c => new NodePortResponse(c.Key, c.DisplayName, c.Description, c.Type.ToString(), c.Required, c.DefaultValue, c.AllowedValues, c.OptionsSource, c.OptionDescriptions)).ToList()
        )).ToList();
