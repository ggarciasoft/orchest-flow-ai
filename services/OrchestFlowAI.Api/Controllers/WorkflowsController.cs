...
public sealed class WorkflowsController : ControllerBase
{
    ...
    {
        var workflow = Workflow.Create(TenantId, req.Name, req.Description, UserId, (OrchestFlowAI.Domain.Enums.TriggerType)(int)req.TriggerType, req.WebhookSecret, req.CronExpression);
        // Apply retry policy when provided
        if (req.RetryMaxAttempts > 0)
            workflow.SetRetryPolicy(OrchestFlowAI.Domain.ValueObjects.RetryPolicy.Create(req.RetryMaxAttempts, req.RetryBackoffMs, req.RetryBackoffMultiplier));
        ...
        return CreatedAtAction(nameof(Get), new { id = workflow.Id }, new WorkflowResponse(workflow.Id, workflow.Name, workflow.Description, 1, workflow.CreatedAt, workflow.UpdatedAt, (OrchestFlowAI.Contracts.TriggerType)(int)workflow.TriggerType, workflow.WebhookSecret, workflow.CronExpression, workflow.RetryPolicy.MaxAttempts, workflow.RetryPolicy.BackoffMs, workflow.RetryPolicy.BackoffMultiplier));
    }
    ...
    public async Task<ActionResult<WorkflowResponse>> Get(Guid id, CancellationToken ct)
    {
        ...
        if (w == null) return NotFound();
        var activeVersion = await _workflows.GetActiveVersionAsync(id, ct);
        return Ok(new WorkflowResponse(w.Id, w.Name, w.Description, activeVersion?.VersionNumber, w.CreatedAt, w.UpdatedAt, (OrchestFlowAI.Contracts.TriggerType)(int)w.TriggerType, w.WebhookSecret, w.CronExpression, w.RetryPolicy.MaxAttempts, w.RetryPolicy.BackoffMs, w.RetryPolicy.BackoffMultiplier));
    }
    ...
    public async Task<ActionResult> Validate(Guid id, CancellationToken ct)
    {
        ...
        var def = JsonSerializer.Deserialize<OrchestFlowAI.Engine.Models.WorkflowDefinition>(activeVersion.DefinitionJson, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
